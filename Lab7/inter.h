...
void genInterCodes(pNode node) {
    if (node == NULL) return;
    if (!strcmp(node->typeName, "ExtDefList"))
...
void translateExtDef(pNode node) {
    ...
    if (!strcmp(node->child->next->typeName, "FunDec")) {
...
void translateFunDec(pNode node) {
    ...
    genInterCode(IR_FUNCTION,
                 newOperand(OP_FUNCTION, newString(node->child->tokenContent)));
    ...
    pItem funcItem = searchTableItem(table, node->child->tokenContent);
...
void translateCompSt(pNode node) {
    ...
    if (!strcmp(temp->typeName, "DefList")) {
        translateDefList(temp);
        temp = temp->next;
    }
    if (!strcmp(temp->typeName, "StmtList")) {
...
void translateVarDec(pNode node, pOperand place) {
    ...
    if (!strcmp(node->child->typeName, "ID")) {
        pItem temp = searchTableItem(table, node->child->tokenContent);
...
void translateStmt(pNode node) {
    ...
    if (!strcmp(node->child->typeName, "Exp")) {
        translateExp(node->child, NULL);
    }

    // Stmt -> CompSt
    else if (!strcmp(node->child->typeName, "CompSt")) {
        translateCompSt(node->child);
    }

    // Stmt -> RETURN Exp SEMI
    else if (!strcmp(node->child->typeName, "RETURN")) {
        pOperand t1 = newTemp();
    ...
    else if (!strcmp(node->child->typeName, "IF")) {
    ...
    else if (!strcmp(node->child->typeName, "WHILE")) {
        pOperand label1 = newLabel();
        pOperand label2 = newLabel();
        pOperand label3 = newLabel();
...
void translateExp(pNode node, pOperand place) {
    ...
    // Exp -> LP Exp RP
    if (!strcmp(node->child->typeName, "LP"))
        translateExp(node->child->next, place);

    else if (!strcmp(node->child->typeName, "Exp") ||
             !strcmp(node->child->typeName, "NOT")) {
        // 条件表达式 和 基本表达式
        if (strcmp(node->child->next->typeName, "LB") &&
            strcmp(node->child->next->typeName, "DOT")) {
            // Exp -> Exp AND Exp
            //      | Exp OR Exp
            //      | Exp RELOP Exp
            //      | NOT Exp
            if (!strcmp(node->child->next->typeName, "AND") ||
                !strcmp(node->child->next->typeName, "OR") ||
                !strcmp(node->child->next->typeName, "RELOP") ||
                !strcmp(node->child->typeName, "NOT")) {
                pOperand label1 = newLabel();
                pOperand label2 = newLabel();
                ...
                // Exp -> Exp ASSIGNOP Exp
                if (!strcmp(node->child->next->typeName, "ASSIGNOP")) {
                    pOperand t2 = newTemp();
                    translateExp(node->child->next->next, t2);
                    pOperand t1 = newTemp();
                    ...
                    translateExp(node->child->next->next, t2);
                    // Exp -> Exp PLUS Exp
                    if (!strcmp(node->child->next->typeName, "PLUS")) {
                        genInterCode(IR_ADD, place, t1, t2);
                    }
                    // Exp -> Exp MINUS Exp
                    else if (!strcmp(node->child->next->typeName, "MINUS")) {
                        genInterCode(IR_SUB, place, t1, t2);
                    }
                    // Exp -> Exp STAR Exp
                    else if (!strcmp(node->child->next->typeName, "STAR")) {
                        genInterCode(IR_MUL, place, t1, t2);
                    }
                    // Exp -> Exp DIV Exp
                    else if (!strcmp(node->child->next->typeName, "DIV")) {
        ...
        // 数组和结构体访问
            ...
            if (!strcmp(node->child->next->typeName, "LB")) {
                ...
                if (node->child->child->next &&
                    !strcmp(node->child->child->next->typeName, "LB")) {
                    //多维数组，报错
                    interError = TRUE;
                    printf(
                        "Cannot translate: Code containsvariables of "
            ...
            // Exp -> Exp DOT ID
                ...
                pOperand id = newOperand(
                    OP_VARIABLE, newString(node->child->next->next->tokenContent));
                int offset = 0;
    ...
    //单目运算符
    // Exp -> MINUS Exp
    else if (!strcmp(node->child->typeName, "MINUS")) {
        pOperand t1 = newTemp();
        translateExp(node->child->next, t1);
    ...
    // Exp -> ID LP Args RP
    //		| ID LP RP
    else if (!strcmp(node->child->typeName, "ID") && node->child->next) {
        pOperand funcTemp =
            newOperand(OP_FUNCTION, newString(node->child->tokenContent));
        // Exp -> ID LP Args RP
        if (!strcmp(node->child->next->next->typeName, "Args")) {
            pArgList argList = newArgList();
            translateArgs(node->child->next->next, argList);
            if (!strcmp(node->child->tokenContent, "write")) {
        ...
        // Exp -> ID LP RP
            ...
            if (!strcmp(node->child->tokenContent, "read")) {
    ...
    // Exp -> ID
    else if (!strcmp(node->child->typeName, "ID")) {
        pItem item = searchTableItem(table, node->child->tokenContent);
        // 根据讲义，因为结构体不允许赋值，结构体做形参时是传址的方式
        interCodeList->tempVarNum--;
        if (item->field->isArg && item->field->type->kind == STRUCTURE) {
            setOperand(place, OP_ADDRESS, (void*)newString(node->child->tokenContent));
            // place->isAddr = TRUE;
        }
        // 非结构体参数情况都当做变量处理
        else {
            setOperand(place, OP_VARIABLE, (void*)newString(node->child->tokenContent));
        }
        ...
        // Exp -> INT
        interCodeList->tempVarNum--;
        setOperand(place, OP_CONSTANT, (void*)atoi(node->child->tokenContent));
        // pOperand t1 = newOperand(OP_CONSTANT, node->child->val);
        // genInterCode(IR_ASSIGN, place, t1);
    }
}
...
void translateCond(pNode node, pOperand labelTrue, pOperand labelFalse) {
    ...
    // Exp -> NOT Exp
    if (!strcmp(node->child->typeName, "NOT")) {
        translateCond(node->child->next, labelFalse, labelTrue);
    }
    // Exp -> Exp RELOP Exp
    else if (!strcmp(node->child->next->typeName, "RELOP")) {
        pOperand t1 = newTemp();
        pOperand t2 = newTemp();
        translateExp(node->child, t1);
        translateExp(node->child->next->next, t2);
        ...
        pOperand relop =
            newOperand(OP_RELOP, newString(node->child->next->tokenContent));
    ...
    // Exp -> Exp AND Exp
    else if (!strcmp(node->child->next->typeName, "AND")) {
        pOperand label1 = newLabel();
    ...
    // Exp -> Exp OR Exp
    else if (!strcmp(node->child->next->typeName, "OR")) {
        pOperand label1 = newLabel();
