...
void traverseTree(pNode node) {
    ...
    if (!strcmp(node->typeName, "ExtDef")) ExtDef(node);
...
void ExtDef(pNode node) {
    ...
    char* secondName = node->child->next->typeName;
...
pType Specifier(pNode node) {
    ...
    pNode t = node->child;
    // Specifier -> TYPE
    if (!strcmp(t->typeName, "TYPE")) {
        if (!strcmp(t->tokenContent, "float")) {
...
pType StructSpecifier(pNode node) {
    ...
    // StructSpecifier->STRUCT OptTag LC DefList RC
    // printTreeInfo(t, 0);
    if (strcmp(t->typeName, "Tag")) {
        // addStructLayer(table);
        pItem structItem =
            newItem(table->stack->curStackDepth,
                    newFieldList("", newType(STRUCTURE, NULL, NULL)));
        if (!strcmp(t->typeName, "OptTag")) {
            setFieldListName(structItem->field, t->child->tokenContent);
            t = t->next;
        }
        // unnamed struct
        else {
        ...
        if (!strcmp(t->next->typeName, "DefList")) {
        ...
        if (checkTableItemConflict(table, structItem)) {
            ...
            if (!strcmp(node->child->next->typeName, "OptTag")) {
    ...
    // StructSpecifier->STRUCT Tag
        ...
        pItem structItem = searchTableItem(table, t->child->tokenContent);
        if (structItem == NULL || !isStructDef(structItem)) {
            char msg[100] = {0};
            sprintf(msg, "Undefined structure \"%s\".", t->child->tokenContent);
...
pItem VarDec(pNode node, pType specifier) {
    ...
    pItem p = newItem(table->stack->curStackDepth, newFieldList(id->tokenContent, NULL));
    ...
    // VarDec -> VarDec LB INT RB
        ...
        while (varDec->next) {
            ...
            // printf("temp type: %d\n", temp->kind);
            p->field->type =
                newType(ARRAY, copyType(temp), atoi(varDec->next->next->tokenContent));
...
void FunDec(pNode node, pType returnType) {
    ...
    pItem p =
        newItem(table->stack->curStackDepth,
                newFieldList(node->child->tokenContent,
                             newType(FUNCTION, 0, NULL, copyType(returnType))));

    // FunDec -> ID LP VarList RP
    if (!strcmp(node->child->next->next->typeName, "VarList")) {
...
void Stmt(pNode node, pType returnType) {
    ...
    pType expType = NULL;
    // Stmt -> Exp SEMI
    if (!strcmp(node->child->typeName, "Exp")) expType = Exp(node->child);
...
pType Exp(pNode node) {
    ...
    //  printTable(table);
    //二值运算
    if (!strcmp(t->typeName, "Exp")) {
        // 基本数学运算符
        if (strcmp(t->next->typeName, "LB") && strcmp(t->next->typeName, "DOT")) {
            pType p1 = Exp(t);
            pType p2 = Exp(t->next->next);
            pType returnType = NULL;

            // Exp -> Exp ASSIGNOP Exp
            if (!strcmp(t->next->typeName, "ASSIGNOP")) {
                //检查左值
                pNode tchild = t->child;

                if (!strcmp(tchild->typeName, "FLOAT") ||
                    !strcmp(tchild->typeName, "INT")) {
                    //报错，左值
                    pError(LEFT_VAR_ASSIGN, t->lineno,
                           "The left-hand side of an assignment must be "
                           "avariable.");

                } else if (!strcmp(tchild->typeName, "ID") ||
                           !strcmp(tchild->next->typeName, "LB") ||
                           !strcmp(tchild->next->typeName, "DOT")) {
                    if (!checkType(p1, p2)) {
                        //报错，类型不匹配
        ...
        // 数组和结构体访问
            ...
            if (!strcmp(t->next->typeName, "LB")) {
                ...
                if (!p1) {
                    ...
                    //报错，非数组使用[]运算符
                    char msg[100] = {0};
                    sprintf(msg, "\"%s\" is not an array.", t->child->tokenContent);
                    ...
                    sprintf(msg, "\"%s\" is not an integer.",
                            t->next->next->child->tokenContent);
            ...
            // Exp -> Exp DOT ID
                ...
                if (!p1 || p1->kind != STRUCTURE ||
                    ...
                    while (structfield != NULL) {
                        if (!strcmp(structfield->name, ref_id->tokenContent)) {
                            break;
                        }
                        structfield = structfield->tail;
                    }
    ...
    // Exp -> MINUS Exp
    //      | NOT Exp
    else if (!strcmp(t->typeName, "MINUS") || !strcmp(t->typeName, "NOT")) {
        pType p1 = Exp(t->next);
        pType returnType = NULL;
        if (!p1 || p1->kind != BASIC) {
            //报错，数组，结构体运算
        ...
        if (p1) deleteType(p1);
        return returnType;
    } else if (!strcmp(t->typeName, "LP")) {
        return Exp(t->next);
    }
    // Exp -> ID LP Args RP
    //		| ID LP RP
    else if (!strcmp(t->typeName, "ID") && t->next) {
        pItem funcInfo = searchTableItem(table, t->tokenContent);

        // function not find
        if (funcInfo == NULL) {
            char msg[100] = {0};
            sprintf(msg, "Undefined function \"%s\".", t->tokenContent);
            pError(UNDEF_FUNC, node->lineno, msg);
            return NULL;
        } else if (funcInfo->field->type->kind != FUNCTION) {
            char msg[100] = {0};
            sprintf(msg, "\"%s\" is not a function.", t->tokenContent);
            pError(NOT_A_FUNC, node->lineno, msg);
            return NULL;
        }
        // Exp -> ID LP Args RP
        else if (!strcmp(t->next->next->typeName, "Args")) {
    ...
    // Exp -> ID
    else if (!strcmp(t->typeName, "ID")) {
        pItem tp = searchTableItem(table, t->tokenContent);
        if (tp == NULL || isStructDef(tp)) {
            char msg[100] = {0};
            sprintf(msg, "Undefined variable \"%s\".", t->tokenContent);
